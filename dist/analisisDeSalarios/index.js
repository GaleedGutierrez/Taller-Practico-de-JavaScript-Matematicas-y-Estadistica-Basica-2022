...
const medianSalaryCompanies = (business, year) => {
...
const salaryProjectionCompanies = (name) => {
    if (!COMPANIES[name])
        return 'Check company name.';
    const SALARIES_YEAR_COMPANY = COMPANIES[name];
    const SALARIES_MEDIAN = {};
...
const getFutureSalary = (salaries) => {
...
const medianTopTen = () => {
    const MEDIAN_PER_PEOPLE = SALARIES.map(person => medianWorks(person.id));
    const TOP_TEN_LENGTH = (!(MEDIAN_PER_PEOPLE.length * 0.1 < 1))
